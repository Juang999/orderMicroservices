...
class VisitController {
    ...
    visitation = async (req, res) => {
        try {
            ...
            let data = await Promise.all([this.getTotalCheckin(sales.user_ptnr_id), this.resultVisitation(sales.user_ptnr_id)])
    ...
    visitation_schedule = async (req, res) => {
        try {
    ...
    detailInvitation = async (req, res) => {
        try {
    ...
    getDataSOforSQ = async (req, res) => {
        try {
            let {periode_start_date, periode_end_date} = await this.currentPeriode(req.query.periode_code)
    ...
    getGoal = async (req, res) => {
        try {
            let current_periode = this.currentPeriode(req.query.periode_code)
            let data_sales = this.getSales(req.params.userid)
    ...
    getVisitationCodeSales = (req, res) => {
        VisitMstr.findAll({
            attributes: [
                'visit_code', 
    ...
    addNewCustomerToVisit = async (req, res) => {
        try {
            let user = await helper.auth(req.get('authorization'))
            let dataCustomerToVisit = await VisitedDet.create({
                            visited_oid: uuidv4(),
                            visited_visit_code: req.body.visit_code,
                            visited_type: req.body.type,
                            visited_ptnr_id: parseInt(req.body.ptnr_id),
    ...
    createSchedule = async (req, res) => {
        try {
            ...
            let checkBeforeCreatePlanningSchedule = await Promise.all([this.checkSchedule(req.body.visit_sales_id, req.body.visit_startdate), this.getPersonalAccountSales(req.body.visit_sales_id)])
            ...
            // generate visit code
            let visit_code = await this.generateVisitCode(en_id)
            ...
            let planningSchedule = await VisitMstr.create({
                ...
                visit_en_id: parseInt(en_id),
                visit_sales_id: parseInt(req.body.visit_sales_id),
    ...
    /*
    Kumpulan dari function di bawah ini adalah untuk memabantu function utama
    */

    checkSchedule = async (sales_id, start_date) => {
    ...
    getPersonalAccountSales = async (sales_id) => {
    ...
    resultVisitation = async (ptnr_id) => {
    ...
    updateStatusSchedule = async (visit_code) => {
        let statusScedule = await VisitMstr.findOne({
            where: {
                visit_code: visit_code
            }
        })

        if (statusScedule.visit_status == 'Y') {
